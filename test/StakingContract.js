...
describe("Staking Contract", function () {
  ...
  async function stake(invester, amount) {
    await testToken
      .connect(invester)
      .approve(stakingContract.getAddress(), amount);
  ...
  describe("Deployment", function () {
    it("should deploy Staking Contract", async function () {
      ...
      stakingContract = await stakingContractFactory.deploy(
        ...
        yieldSupplier.address,
        2,
        "1000000000000"
      );

      await testToken
        .connect(yieldSupplier)
        .approve(stakingContract.getAddress(), yieldLimit);
    });
  });
  describe("Stake", function () {
    ...
    it("should be able to stake multiple times", async function () {
      ...
      await stake(invester1, supplyAmountInvester);
      expect(
        (await stakingContract.stakes(invester1.address)).stakedAmount
      ).to.equals(supplyAmountInvester);

      await stake(invester1, supplyAmountInvester);
      expect(
        (await stakingContract.stakes(invester1.address)).stakedAmount
      ).to.equals(ethers.parseEther("2"));

      await stake(invester1, supplyAmountInvester);
      expect(
        (await stakingContract.stakes(invester1.address)).stakedAmount
      ).to.equals(ethers.parseEther("3"));
    });

    it("should not be able to stake tokens if amount or address is zero", async function () {
      await testToken
        .connect(invester1)
        .approve(stakingContract.getAddress(), supplyAmountInvester);
      await expect(
        stakingContract
          .connect(invester1)
          .stake(ethers.ZeroAddress, supplyAmountInvester)
      ).to.be.revertedWithCustomError(stakingContract, "ZeroStaker");
      await expect(
        stakingContract.connect(invester1).stake(invester1.address, 0)
  ...
  describe("Withdraw", function () {
    ...
    it("should be able to stake multiple times and withdraw", async function () {
      ...
      expect(await stakingContract.connect(invester1).withdraw()).to.emit(
        testToken,
        "transfer"
      );
    });
    ...
    it("should not be able to withdraw more than yield limit", async function () {
      ...
      await testToken
        .connect(yieldSupplier)
        .approve(stakingContract.getAddress(), "10000");
      ...
      await expect(
        stakingContract
          .connect(invester1)
          .stake(invester1.address, supplyAmountInvester)
      ).to.be.revertedWithCustomError(stakingContract, "YieldLimitExceeded");
  ...
  describe("Pause", function () {
    ...
    it("should not be able to pause contract if not called by owner", async function () {
      await expect(
        stakingContract.connect(invester1).pauseStaking()
  ...
  describe("Calculate rewards", function () {
    ...
    it("should be able to calculate rewards", async function () {
      ...
      expect(
        await stakingContract.calculateTotalReward(invester1.address)
      ).to.equals(
        (ethers.toBigInt(rewardPerBlock * blocks) * supplyAmountInvester) /
          ethers.parseEther("1")
      );
    });
